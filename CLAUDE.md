...
**🔴 The lockout that caused it — read before touching membership code.** A real maker was locked out of prod for 3 days (denials 08-19/21/22). `syncGarmGrantForEmail` never created their Garm grant — verified zero grant rows, not even a revoked one. The dual-write is fire-and-forget with a swallowed `catch`: correct while Firestore was authoritative, but PR G made Garm the fail-closed authority and silently promoted every swallowed failure into a **permanent silent lockout**. Repaired by hand with one grant upsert (actor `nico`). The reconcile is only a backstop bounding this at ~1h; **the real fix is #174** — make invite/share await its grant and surface failure to the builder. Do NOT instead "fix" it by healing at sign-in: that re-creates the local fallback authority PR G deliberately deleted.
