...
> Shipped history lives in `docs/changelog.md`. This section is **live work only**. The open-issue queue on GitHub (`gh issue list`) is the canonical backlog; key threads: **#104** (email/password auth — next), **#105** (admin Brief-doctor tooling), **#106** (multi-human membership lifecycle), **#84** (JSON-blob/chat authoring pivot — north star), **#107** (preview test-login), **#23** (files folders), **#29** (explore mode), **#39** (prep-prompt ferry), **#40** (realtime drift), **#43** (voice attribution, parked), **#61** (fixtures shims), **#72** (richer prototype perception).

1. **#104 email/password auth + reset — NEXT.** Branch `auth-email-password` exists (empty placeholder); plan scoped; **closed-signup decided** (passwords only set from an authenticated session via `linkWithCredential` to the existing Google UID, or via `sendPasswordResetEmail` — no public registration). Enable Email/Password in Firebase consoles (preview first); login-page password mode + UserMenu set-password + forgot-password. Fixes Nico's wrong-Google-account friction.
2. **#105 admin Brief-doctor — curated, non-destructive conversation-state tooling.** Design settled: curated ops (not raw editor), env-agnostic, audit log ON (`admin_actions`), **no hard deletes — `archived` flag instead**. Seed already built: `scripts/fix-reopen-conversation.mjs` (reopen previous conversation; used to fix a real BySide accident). Wrap in `/admin/briefs` + `/api/admin/sessions`. Follow-on: session-list GET should filter `status:'archived'`.
3. **#106 multi-human Phase 2 — membership lifecycle.** Member "move out" (archive membership, non-destructive) + access-tier change + dual-role + attribution polish. Cast fixture: `scripts/seed-test-cast.mjs`.
...
6. **⚠️ Preview gotchas.** (a) Vercel SSO gate is ON — agent uses `.ibuild4you-bypass` + `.test-admin-passcode` (preview) / `.test-admin-passcode-prod` (prod) for headless Playwright. (b) Google sign-in on preview 400s (`redirect_uri_mismatch`) — never wired; passcode login works (fix in #107).
