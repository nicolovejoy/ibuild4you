...
**Prompt caching for the chat agent — SHIPPED 2026-09-18** (PR #178, `c5529b1`; CI + post-deploy smoke + auth e2e green, prod health 200; details in `docs/changelog.md`). **Unconfirmed:** a steady-state cache hit was never read back — the preview test's first pair missed for the known gap-block reason, and the second pair's `check-cache-read` numbers weren't checked before merge. **Verify on prod:** after some real maker traffic, `api_usage` rows for route `chat` should show `cache_read_input_tokens` ≫ `cache_creation_input_tokens` from the third turn of a visit onward; if every row reads 0, the prefix isn't stable — debug first with `scripts/check-cache-read.mjs` (judges the newest turn) on a preview session. **Follow-up PR:** move volatile system-prompt content (the ≥1hr "coming back after a gap" block, prototype age labels) into a second, unmarked system block after the cached one — today every return visit pays two full ~24k cache writes before caching saves anything (turn 1 writes; turn 2 misses because the gap block dropped out). **Not this repo:** bakerylouise_v1 is the one Vercel project still on Node 20.x — builds fail from 2026-09-30.
