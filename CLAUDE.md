...
> Shipped history lives in `docs/changelog.md`. This section is **live work only**. The open-issue queue on GitHub (`gh issue list`) is the canonical backlog; key threads: **#72** (richer prototype perception — next, needs deep-think), **#84** (JSON-blob/chat authoring pivot — north star), **#83** (unified artifacts store, subsumes #23 folders), **#23** (files folders), **#29** (explore mode), **#39** (prep-prompt ferry), **#61** (fixtures shims), **#110** (dual-role, deferred from #106), **#107** (Google-OAuth-on-preview owner step). Parked/deferred: **#43** (voice attribution). **#104 (auth) + #105 (Brief-doctor) + #107 preview e2e helper + #106 (membership lifecycle) shipped; #40 closed as accepted debt.**

1. **#72 richer prototype perception — NEXT, DEEP-THINK FIRST (use a top model).** Plan scaffold: `docs/prototype-perception-plan.md` — do NOT code; design the approach first. Loop→agent *text* channel already ships (`lib/api/prototype-feedback.ts` feeds recent feedback into chat/kickoff prompts); remaining work is richer visual/structural capture + how the agent consumes it (vision path?) + data-path/collection + adoption pilot (byside) + privacy. Direction is settled: extend Loop, not headless browsing.
