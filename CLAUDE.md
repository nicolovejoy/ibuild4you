...
2. **Reminders → admin feature → flip live.** PR #22's dry-run window expires when the system becomes self-observable. Three steps:
   - (a) **Per-brief `auto_remind_enabled` toggle.** New boolean field on `projects` (default `true`). Surface in Setup tab + small indicator on dashboard rows. Cron at `/api/cron/maker-reminders` reads it and skips disabled briefs.
   ...
   - (d) **Flip live.** Delete `REMINDER_DRY_RUN` on Vercel. With (a)+(b)+(c) live, regressions surface on `/admin/reminders` instead of needing the env-var safety switch.
