...
1. **#72 B3/B5 — watch the byside pilot, then decide.** Capture is live on by-side.net (structure rides feedback submissions into `prototype_context` → Sam's prompt). B3 (capture-only "Show Sam this page" button + admin capture chip) ships only if makers want capture outside feedback submissions; B5 (explicit `getDisplayMedia` screenshots) only if structure proves insufficient. Design + slices: `docs/prototype-perception-plan.md` §4.
2. **#107 preview test-login — code done; one owner step remains.** Shared login helper (`scripts/lib/preview-login.mjs`) + docs shipped. Remaining: wire Google OAuth on preview (owner-only console work — see #107 for the exact Firebase/GCP steps). Passcode login already works; Google is only for a human eyeballing preview.
