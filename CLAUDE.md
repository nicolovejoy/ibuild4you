...
> Shipped history lives in `docs/changelog.md`. This section is **live work only**. The open-issue queue on GitHub (`gh issue list`) is the canonical backlog; key threads: **#84** (JSON-blob/chat authoring pivot — north star), **#83** (unified artifacts store, subsumes #23 folders), **#23** (files folders), **#29** (explore mode), **#39** (prep-prompt ferry), **#61** (fixtures shims), **#110** (dual-role, deferred from #106), **#107** (Google-OAuth-on-preview owner step), **#116** (PAT rotation reminder, act ~Jun 2027). Parked/deferred: **#43** (voice attribution), **#72 B3/B5** (gated on byside pilot signal). **#72 core (design + B1+B2+B6 + byside pilot) shipped 2026-07-04.**

1. **#121 — decision provenance. NEXT.** Stamp `decided_in_session`/`decided_at` on `brief.decisions[]` at regen time (additive schema). Byside seat confirmed immediate value ("when did we lock X" currently answered by grepping transcripts; hand-copied decisions demonstrably drift). Unlocks #122's round headers.
...
4. **#107 preview test-login — code done; one owner step remains.** Shared login helper (`scripts/lib/preview-login.mjs`) + docs shipped. Remaining: wire Google OAuth on preview (owner-only console work — see #107 for the exact Firebase/GCP steps). Passcode login already works; Google is only for a human eyeballing preview.
...
7. **⚠️ Preview gotchas.** (a) Vercel SSO gate is ON — agent uses `.ibuild4you-bypass` + `.test-admin-passcode` (preview) / `.test-admin-passcode-prod` (prod) for headless Playwright. (b) Google sign-in on preview 400s (`redirect_uri_mismatch`) — never wired; passcode login works (fix in #107). (c) **Email/Password is enabled on both Firebase projects** (`ibuild4you-preview` + `ibuild4you-a0c4d`) as of 2026-07-01 — password login works on preview + prod.
