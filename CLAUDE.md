...
> Shipped history lives in `docs/changelog.md`. This section is **live work only**. The open-issue queue on GitHub (`gh issue list`) is the canonical backlog; key threads: **#106** (multi-human membership lifecycle — next), **#107** (preview test-login), **#84** (JSON-blob/chat authoring pivot — north star), **#23** (files folders), **#29** (explore mode), **#39** (prep-prompt ferry), **#40** (realtime drift), **#43** (voice attribution, parked), **#61** (fixtures shims), **#72** (richer prototype perception). **#104 (auth) + #105 (Brief-doctor) shipped 2026-07-01.**

1. **#106 multi-human Phase 2 — membership lifecycle — NEXT.** Member "move out" (archive membership, non-destructive) + access-tier change + dual-role + attribution polish. Cast fixture: `scripts/seed-test-cast.mjs`. Follows the same no-hard-delete/archive pattern as #105.
2. **#107 preview test-login + Google-on-preview fix.** Small; unblocks smoother preview testing. Google sign-in on preview still 400s (`redirect_uri_mismatch`) — never wired; passcode login works.
