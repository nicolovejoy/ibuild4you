...
- **Who speaks first.** Only a brief's *first* session gets the canned `welcome_message` (#70) — a `welcome_message` set later is never posted. On session 2+ the agent greets via kickoff (`lib/agent/kickoff.ts` decides client-side, `/api/chat/kickoff` is the authority): always on an **empty** return session with prior maker history (PR #177), otherwise only ≥1hr after the maker's last message. Kickoff is maker-only — builder-side roles (incl. admins, who resolve to `owner`) never trigger it.
...
> Shipped history lives in `docs/changelog.md` (newest first). **Prepend an entry there at session end — routine `/handoff` no longer touches docs (it only saves the session row), so do it explicitly before closing.** This section is **live work only**. Canonical backlog: `gh issue list --state open`.
...
**stars-demo round-two integration MERGED 2026-09-22 (PR #179, `c8bbe34`).** Server-to-server group-conversation API under `app/api/integrations/stars-demo/*`; plan `docs/superpowers/plans/2026-09-22-stars-demo-round-two-integration.md`, follow-up `docs/superpowers/plans/2026-09-22-stars-demo-post-merge.md`. **Three post-merge steps still open (Nico):** (1) `STARS_INTEGRATION_SECRET` on Vercel preview + prod — until then every integration request is a 401 by design; (2) `firebase deploy --only firestore:indexes` on both Firebase projects (three new `integration_messages` indexes); (3) run the smoke script. **All three DONE 2026-09-22 13:xx PT:** secret on Production only (preview has none, so the smoke runs against prod with `--allow-prod`), indexes deployed, `scripts/smoke-stars-integration.mjs` passed 29/29 on prod once the composite indexes finished building (a run 3 min after deploy saw `503 unavailable` on every list — Firestore builds composites asynchronously). stars-demo can point at prod. Namespace and round are hard-wired to `stars-demo`/`r2`; a second tenant or round three generalises those two constants first.
...
**Reliability + lockout follow-through SHIPPED 2026-08-22/23** (PRs #175, #176). Fail-closed cron auth (`lib/api/cron-auth.ts`), `garmProbe()` → `garm_reachable` in `/api/health`, hourly `/api/cron/garm-reconcile` (additive-only, cap 10, gated on `GARM_DUAL_WRITE`), a counts-only Garm denial classifier (`lib/garm-denials.ts`, `garm_denials` collection, `denials_24h_*` on the reconcile row), and the invite POST now awaits its grant (#174). Plans: `docs/reliability-hardening-plan.md`, `docs/superpowers/plans/2026-08-22-garm-denial-detector-and-await-invite-grant.md`. **The maker lockout was an identity mismatch (aliasing #169, case 2), not a dropped dual-write** — stray grant revoked, alias created (`scripts/garm-revoke-grant.mjs` exists for the revoke step; alias via `POST /api/aliases` with `GARM_ADMIN_KEY`). **Watch next:** first `garm_reconcile_log` rows (`error:null`, `extra_count:0`, denial counts), `/api/health` stays 200 on prod, and the maker's Google sign-in landing. Never "fix" an aliasing case by adding a second `project_members` row — it splits one human into two roster participants. **The `garm_reachable` check (`garmProbe()`) was removed 2026-08-23** — it was generating ~200/205 of Garm's denial digest per window pinging a fake principal; see `docs/changelog.md` for the removal. `/api/health` no longer checks Garm at all. **Only remaining open Garm thread: listing-contract confirmation** (`GET /api/grants?project=X` → `{grants:[...]}`, active-only, unpaginated).

**GitHub Actions synthetic monitoring retired 2026-08-24, replaced with UptimeRobot.** `.github/workflows/monitor.yml` (10-min cron hitting `/api/health`) was burning ~750+/2000 Actions minutes/month account-wide (flagged by prompt-lab), deleted outright (`4b6c050`). UptimeRobot monitor "ibuild4you production health" now polls `https://ibuild4you.com/api/health` every 5 min (free tier) instead — verified unauthenticated, real-404-on-bad-path, no DB-autosuspend concern first. **Double-check Alert Contacts are actually assigned on the monitor** — an unassigned monitor shows green and pages nobody, this bit prompt-lab before.

**Blank-second-conversation fix SHIPPED 2026-09-17** (PR #177) — hit live during an interview exercise on brief `nms-head-of-technology-first-six-months`; details in `docs/changelog.md`. Diagnose "can't open the conversation" reports with `scripts/inspect-project.mjs` (read-only; run via `op run --env-file=.env.ops.tpl -- node scripts/with-prod-env-ro.mjs …` — the laptop's `.env.local` service account has no `private_key`, so `with-prod-env.mjs` alone fails there). **Follow-ups, none started:** (1) drive the kickoff fix end-to-end on preview — needs the seeded test cast (`.test-cast-passwords.json` via `scripts/fixtures/scenarios/multi-human-cast.mjs`) and a short maker-side e2e; (2) a static empty-state hint in the maker chat for when the kickoff call itself fails (still a blank screen today); (3) decide whether a builder-set `welcome_message` should post on session 2+ — today it is silently unused, which is what made this look like a payload mistake.

**Prompt caching for the chat agent — SHIPPED 2026-09-18** (PR #178, `c5529b1`; CI + post-deploy smoke + auth e2e green, prod health 200; details in `docs/changelog.md`). **Steady-state hit CONFIRMED on preview** (2026-09-19 00:06Z, session `DCGnv2LPyd6VEWxX8t9f`): turn 1 read 0 / wrote 24,813 (>5 min since the prior turn), turn 2 **read 24,813** / wrote 87. A temporary fingerprint log showed byte-identical system prompt + message prefix across the two turns. **One unexplained miss:** an earlier pair (api_usage turns 17→18) missed on turn 2 and its request grew only ~11 tokens instead of ~85; not a reply-storage race (the reply is persisted before `[DONE]`). If prod `api_usage` for route `chat` shows frequent misses on back-to-back turns, reinstate the fingerprint log (hash + length of system and each message, logged next to `applyPromptCaching` in the chat route) and diff consecutive turns via `vercel logs --expand`. **Follow-up PR:** move volatile system-prompt content (the ≥1hr "coming back after a gap" block, prototype age labels) into a second, unmarked system block after the cached one — today every return visit pays two full ~24k cache writes before caching saves anything (turn 1 writes; turn 2 misses because the gap block dropped out). **Not this repo:** bakerylouise_v1 is the one Vercel project still on Node 20.x — builds fail from 2026-09-30.

**MusicForge Loop project created 2026-09-05** (slug `musicforge`, project id `5Gd1lNXkRwJtJUIbpwlO`) — MusicForge web (PR #400) and iOS (Build 87) were POSTing `projectId: "musicforge"` to `/api/feedback` and 404ing since no matching `projects.slug` existed. Created via the dashboard's Import JSON flow, confirmed present in prod. Reply posted on the `ibuild4you-musicforge` handoff channel; **awaiting their smoke-test confirmation** (iPad Build 87 → Settings → Send feedback, and www.musicforge.org ladybug widget) that submissions now land in `/admin/feedback`.
...
- `STARS_INTEGRATION_SECRET` — shared secret for stars-demo's server-to-server calls to `/api/integrations/stars-demo/*` (header `X-Integration-Secret` + `X-Integration-Namespace: stars-demo`). Fail-closed: unset denies every request. Constant-time compare over SHA-256 digests. One value at a time, rotation is a coordinated cutover (08a §8). Value in 1Password `op://dev-secrets/starsdemo-ibuild4you-integration-secret/credential` (API Credential item; field is `credential`, verified 2026-09-22) (same value in stars-demo's `IBUILD4YOU_INTEGRATION_SECRET`). Never logged.
...
- `GARM_DUAL_WRITE` — kill switch for **both** the dual-write and the reconcile cron (Garm Phase 4). Must be **exactly `'on'`** to enable; unset or any other value → off, and neither `lib/garm-grants.ts` nor `/api/cron/garm-reconcile` calls Garm at all. Off means Firestore and Garm are allowed to diverge by operator intent, so the reconcile stands down rather than healing that divergence back — but it still writes a `garm_reconcile_log` row each run with `skipped_dual_write_off: true`, so a forgotten flag shows up hourly instead of silently disabling the lockout safety net. When on, every membership/approved-email write (create, share/invite, role change, remove, restore, rekey) recomputes that email's app-level role and upserts or revokes its Garm grant via Next's `after()` — fire-and-forget, local Firestore stays the source of truth, a Garm failure is logged (booleans/role only, never the email) and never blocks or unwinds the local write.
...
- **No bare backslash in a copy-paste command block.** A `\` is load-bearing shell syntax that renders invisibly and gets silently dropped somewhere between the markdown render, the clipboard, and zsh. `find … -exec test -e {} \; -delete` arrived in the terminal as `… {} ; -delete`, which zsh split into two commands and reported as `find: -exec: no terminating ";"` plus `command not found: -delete` (2026-09-20). Quote it instead — `';'` is exactly equivalent to `\;` and survives any copy path. For the same reason never break a command across lines with a trailing `\`.

- **Keep every line in a copy-paste block under ~75 characters.** Nico's terminal wraps rendered code at about 85 columns and the wrap is copied as a real newline, so zsh runs the fragments as separate commands. On 2026-09-22 a 100-character `export X=$(op read 'op://…')` line arrived as `op read` with no argument, three pastes in a row. Never fix this with a trailing `\` (see above); split the work into short lines instead — put a long path or reference in a shell variable on its own line (`REF=op://vault/item/field`), use `$REF` on the next line, and `export` env vars one per line before the command that needs them.
