...
1. **#121 — decision provenance. NEXT — plan DECIDED at `docs/decision-provenance-plan.md` (2026-07-11), build from that doc.** Stamp `decided_in_session` (session id) + `decided_at` on `brief.decisions[]` — one pure `stampDecisionProvenance` in `lib/api/brief-merge.ts` used by BOTH regen and the PUT /api/briefs paste path (paste also restores stamps the ferry drops); model-emitted provenance is stripped on regen. Unlocks #122's round headers.
2. **#122 — `inherits_from` + round-timeline view. Gated, not scheduled.** Inherit-not-merge decided 2026-07-10 (locked decisions + constraints travel with provenance; old brief stays archived/linkable). Gates: inheritance at the next new-brief-with-predecessor moment (live test pair exists in the byside family); timeline only if briefs live past ~5 rounds.
