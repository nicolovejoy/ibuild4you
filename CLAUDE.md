...
16. **Matt/BySide maker feedback — 3 SHIPPED to prod, #70 next (2026-06-15).** Triaged a real multi-session intake transcript into 4 issues. **#69 SHIPPED (PR #73, `d89476c`):** agent self-awareness — declares "I'm intake, hand to your developer" up front (DEFAULT_IDENTITY + first-session intro + welcome generator) + admits it can't see the running site (offers screenshot path) instead of faking a walkthrough; both modes via GUARDRAILS. **#74 SHIPPED (PR #74, `9ff1b68`):** `/api/projects/[id]/members` 500 fixed — `getUserDisplayName` guards empty `uid` (a not-yet-signed-in member made Firestore `.doc('')` throw → "Failed to load members" on Setup; hit prod too). **#75 SHIPPED (PR #75, `857fa1a`):** builder emails the maker **directly via Resend** (invite/nudge/reminder) — `POST /api/projects/[id]/email` + `lib/email/send-maker-email.ts` + Modal confirm in `SendToMakerButton`; To: maker, BCC+Reply-To: builder; honors `nudge_message` override; mints invite passcode. Extracted `lib/passcode.ts`, `getServerShareLink`. **NEXT = #70 welcome-replay** (static `welcome_message` re-fires verbatim every session) — plan written: `docs/welcome-replay-plan.md` (approach A: suppress canned welcome on session 2+, let agent recap / relax kickoff gate to project-history). **Also open:** #71 brief↔build reconciliation (MVP slice = durable locked-decisions + forced reconciliation pass; repo-drift bridge later), #72 agent-perceives-prototype via Loop context-capture (NOT headless browsing).
15. **Feedback batch (2026-06-14) — 4 issues filed, prioritized; awaiting clearance to start.** From Nico's reminders list. **P1 = #65 cross-brief notification digest** (the bug: 4 emails for one conversation; decided on one consolidated "briefs needing you" email replacing per-brief notify spam — `app/api/chat/route.ts:11` 5-min debounce + `/api/cron/notify`; no "builder viewed" signal exists so notify-until-seen is out). **P2 = #66 + #67 visibility cluster** (#66: labeled Admin entry in `UserMenu` dropdown, admins only — gear is undiscoverable; #67: surface reminder *status* — `reminders_sent_count`/`last_reminder_sent_at`/next-send — on the Setup tab; the toggle already exists at `BuilderProjectView.tsx:1344`, so mostly UI). **P3 = #68 JSON import tolerance** (normalize smart quotes before `JSON.parse` in `lib/api/import-payload.ts` + prep-prompt guidance; "how pure?" scope still TBD — discuss first). **Side track (not a repo issue): cross-domain usage analytics** — Cloudflare Web Analytics on all zones now (consistent, ~no build), PostHog later only on apps needing "who"/retention; track as personal-infra note, not here.

## Recent context

Full dated history: `docs/changelog.md`. Most recent below.

**Shipped 2026-06-15 (Matt/BySide maker feedback — 3 PRs to prod):** Triaged a real multi-session intake transcript into 4 GitHub issues (#69–72). **#69 (PR #73, `d89476c`):** agent self-awareness — Sam declares "I'm intake, I hand this to your developer" up front (DEFAULT_IDENTITY + first-session intro + welcome generator) and admits it can't see the running prototype (offers screenshot path) instead of faking a walkthrough; guardrails in both modes. **#74 (PR #74, `9ff1b68`):** fixed a prod 500 on `/api/projects/[id]/members` — `getUserDisplayName` now guards an empty `uid` (a not-yet-signed-in member made Firestore `.doc('')` throw → "Failed to load members"). **#75 (PR #75, `857fa1a`):** builders email the maker **directly via Resend** (invite/nudge/reminder) — `POST /api/projects/[id]/email`, `lib/email/send-maker-email.ts`, a Modal-confirm `SendToMakerButton` (replaced a confusing inline confirm); To: maker, BCC+Reply-To: builder; honors `nudge_message` override + mints invite passcode; extracted `lib/passcode.ts` + `getServerShareLink`. 669 green. **Email DNS verified** (sending ✅ via Resend DKIM/SES; DMARC missing — optional add; `test@ibuild4you.com` doesn't receive so preview BCC bounces, prod unaffected). **Next: #70 welcome-replay** — plan at `docs/welcome-replay-plan.md` (approach A).
