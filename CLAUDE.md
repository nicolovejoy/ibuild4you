...
1. **Feedback widget — continue building** (issue #3, backend shipped in PR #5; composite indexes deployed 2026-05-13). Done: (a) admin UI at `/admin/feedback` with project/status/type filters + inline status/notes edit; (b) `<FeedbackWidget projectId />` at `components/FeedbackWidget.tsx` with shared validator/payload helper at `lib/feedback/payload.ts` and RTL smoke tests; (c) `POST /api/admin/feedback/[id]/to-github` route + per-row "Convert to GitHub issue" button — pure helpers in `lib/feedback/github.ts`, idempotent if `github_issue_url` already set. Requires `GITHUB_TOKEN` env var on Vercel and `projects.github_repo` set per project. Remaining: (d) Resend inbound webhook → `feedback/{id}/replies` subcollection (`FeedbackReply` type already defined); (e) embed on Bakery Louise (point its build at the widget — it's plain React with no app-specific deps beyond Tailwind tokens).
2. Validate Matt's PDF flow end-to-end. Cache_control fix (`a8d9c94`) is live but unverified against real Matt traffic — need to see his next chat return 200 AND the agent reference actual PDF content (form name, clause), not just acknowledge files exist. Tune `lib/agent/system-prompt.ts` if it ignores/hallucinates.
3. A4 — pre-upload batch size budgeting. `addFiles` currently checks per-file >25MB; extend to running batch total so the picker rejects before the init round-trip. See `docs/file-and-brief-fixes-plan.md` § A4.
4. Project delete should clean up files. Today DELETE `/api/projects` removes sessions/messages/briefs/members but leaves the `files` Firestore docs and S3 objects orphaned. Reference: `scripts/cleanup-test-data.mjs` — factor its S3+Firestore delete steps into a shared helper.
5. Plan P4/P5 (denormalized session counters + retire `requester_*` legacy fields). Plan file: `~/.claude/plans/zesty-tumbling-fountain.md`. Both gated on telemetry — only worth doing if post-PR-#4 dashboard reads are still high. Check Firestore usage before committing time.
...
8. Add tests for `useStreamingChat` hook. RTL setup is now proven (see `components/__tests__/FeedbackWidget.test.tsx`): use `// @vitest-environment jsdom` pragma, `import '@testing-library/jest-dom/vitest'`, and call `cleanup()` in `afterEach` (RTL doesn't auto-clean with our vitest config).
9. Project folders for the dashboard — group stale projects into folders, badge on each folder with count of projects where it's the builder's turn. Design questions open: per-builder vs shared, one folder vs many, default folder, drag-drop vs menu.
10. Smoke-test the prep-prompts split in prod (commit `21f4c10`). Eight cases listed in prior conversation: happy paths for both flows, mismatch rejection both ways, backward-compat with no `_payload_type`, `seed_questions` round-trip, `open_risks` preservation, code-fence tolerance.
11. Maker experience design exploration. `docs/maker-experience-functionality.md` is the implementation-agnostic functional spec. Next: hand to one or two design agents to propose interface concepts. Open questions in §8.
12. Maker re-engagement flow — signed-token email action links (3/7/14/30 day snooze + opt-out → feedback page). See `docs/maker-re-engagement-plan.md`. Blocked on conversation with Ryan re: snooze values, feedback chips, cadence.
13. Validate Session 4 with Matt using new `voice_sample` + `nudge_message` override. First real test of voice-anchored AI outbound copy and verbatim override.
...
- `GITHUB_TOKEN` — for `/api/admin/feedback/[id]/to-github`. Fine-grained PAT scoped to the `github_repo` repos you'll convert feedback into; needs `Issues: Read & write` permission. Without it the route returns 500. Per-project repo is configured on `projects.github_repo` (e.g. `nicolovejoy/bakery-louise`).
