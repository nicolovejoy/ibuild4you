...
13. **#56 — account identity in top nav + nameable accounts.** Surface signed-in email/name in the always-visible nav (not just behind the user-menu) + let users label their account ("main"/"test"). Nico can't tell which account he's in. New ask 2026-06-13.
14. **Brief-identity follow-ups.** Identity system shipped (color/code/glyph, see memory `project_brief_identity`). Next: #44 dashboard restructure by role/turn-state now has the visual vocabulary to build on; dashboard-card badge still uses the access-tier default for `brief_role` (`lib/api/enrich-projects.ts` doesn't carry it) — low priority.
