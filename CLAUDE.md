...
14. **#44 — dashboard restructure by role/turn-state — PLANNED, awaiting clearance.** Detailed plan at `docs/dashboard-restructure-plan.md`; decisions locked (Hybrid grouping: pinned "Awaiting you" + Yours/Reviewing/Contributing + collapsed Done; Phases 0–2 one PR, filter deferred; everyone gets sections with low-N collapse). **Phase 0 = two free data fixes:** thread `viewer_brief_role` through `enrichProjects` (zero extra reads — also fixes the Contributor-as-Originator badge, the old item-6/14 gap) + add a `state` discriminant to `getTurnIndicator`. Then pure `lib/dashboard/group-briefs.ts` + sectioned UI. **Do not start coding until Nico clears it.**
