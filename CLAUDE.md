...
**Prompt caching for the chat agent — PR #178 OPEN** (2026-09-18, branch `chat-prompt-cache`, worktree `../ibuild4you-chat-prompt-cache`; details in `docs/changelog.md`). **Before merging:** run `scripts/check-cache-read.mjs DCGnv2LPyd6VEWxX8t9f` against preview (via `op run` with the preview tpl + `with-preview-env.mjs`) — the newest turn should show `cache_read` ≈ 24k; if 0, the prefix isn't stable and the PR needs a look. **After merge:** watch `api_usage` for route `chat` (cache_read vs cache_create), then remove the worktree. **Follow-up PR:** move volatile system-prompt content (the ≥1hr "coming back after a gap" block, prototype age labels) into a second, unmarked system block after the cached one — today every return visit pays two full ~24k cache writes before caching saves anything (turn 1 writes; turn 2 misses because the gap block dropped out). **Not this repo:** bakerylouise_v1 is the one Vercel project still on Node 20.x — builds fail from 2026-09-30.
