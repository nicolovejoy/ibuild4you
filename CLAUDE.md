...
- `STARS_INTEGRATION_SECRET` — shared secret for stars-demo's server-to-server calls to `/api/integrations/stars-demo/*` (header `X-Integration-Secret` + `X-Integration-Namespace: stars-demo`). Fail-closed: unset denies every request. Constant-time compare over SHA-256 digests. One value at a time, rotation is a coordinated cutover (08a §8). Value in 1Password `op://dev-secrets/starsdemo-ibuild4you-integration-secret/password` (item title verified 2026-09-22; field name assumed `password`) (same value in stars-demo's `IBUILD4YOU_INTEGRATION_SECRET`). Never logged.
