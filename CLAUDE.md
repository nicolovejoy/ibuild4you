...
0. **URGENT — Deploy Firestore composite indexes**. PR #5 added two `feedback` indexes (project_id+created_at, status+created_at); PR #4 added a `messages` index (session_id+role+sender_email+created_at) for `last_maker_message_at`. Without deploys, `/api/admin/feedback?projectId=...&status=...` will throw FAILED_PRECONDITION and `last_maker_message_at` stays null on the dashboard. From repo root: `npx firebase-tools login` (one-time), then `npx firebase-tools deploy --only firestore:indexes --project ibuild4you-a0c4d`. Indexes build in the Firebase console afterward — check status there before testing.
1. **Feedback widget — continue building** (issue #3, backend shipped in PR #5). Next pieces in order: (a) admin UI at `/admin/feedback` — list with filters, inline status/notes edit; (b) `<FeedbackWidget projectId />` React component (neutral styling, honeypot+`_ts` fields, POSTs to `/api/feedback`); (c) `POST /api/admin/feedback/[id]/to-github` endpoint — reads `projects.github_repo`, creates issue via GitHub API, stores url; (d) Resend inbound webhook → `feedback/{id}/replies` subcollection (`FeedbackReply` type already defined); (e) embed on Bakery Louise.
...
8. Add tests for `useStreamingChat` hook (needs React Testing Library setup — `@testing-library/react` and `jsdom` are now in package.json).
