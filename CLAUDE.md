...
15. **Feedback batch (#65-68) — Phase 0 sweep SHIPPED to PR #77 (open, on preview); #65 is next.** **#66 (admin-menu in `UserMenu`) was already done** — verified, the gated entry is live in `components/user-menu.tsx`. **#67 (reminder status on Setup) + #68 (tolerant JSON import) shipped in PR #77** (`#67`: new `nextReminderAt()` in `lib/api/reminder-cadence.ts` + read-only strip under the toggle; `#68`: `parseLooseJson()` in `lib/utils.ts` — try-parse then structural repair of smart-quote delimiters/NBSP, valid content never rewritten). **PR #77 awaits Nico's preview eyeball + merge.** **NEXT P1 = #65 cross-brief notification digest** (the bug: 4 emails for one conversation; replace per-brief notify spam with one daily "briefs needing you" digest — pull the email send out of the `*/5` `/api/cron/notify` loop into a daily cron, keep its idle-brief-regen half ticking; slice 1 keeps the hardcoded recipient = zero migration). **Side track (not a repo issue): cross-domain usage analytics** — Cloudflare Web Analytics on all zones; PostHog later only where "who"/retention matters.
