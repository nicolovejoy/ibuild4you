...
1. **URGENT — hand-test the to-GitHub button on prod.** PR #7 is merged + Vercel deploy should be live; `GITHUB_TOKEN` is set in Vercel production (Nico, 2026-05-13). Steps: (1) in Firebase console, set `github_repo = nicolovejoy/ibuild4you` (test target — cheapest blast radius) on a project that already has feedback rows, OR submit a test feedback first via curl/the public POST; (2) sign in to ibuild4you.com/admin/feedback, find the row, click "Convert to GitHub issue"; (3) confirm the issue lands at https://github.com/nicolovejoy/ibuild4you/issues with title `[<type>] <body>` + labels `feedback` + `<type>`, and the row swaps to "Open GitHub issue"; (4) click again — should be idempotent (no duplicate issue). To watch logs in parallel: `vercel logs --follow` (run `vercel link` first if not linked). Surface any 4xx/5xx and report back before moving on.
2. **Feedback widget — finish issue #3.** Done: (a) admin UI at `/admin/feedback` with project/status/type filters + inline status/notes edit; (b) `<FeedbackWidget projectId />` at `components/FeedbackWidget.tsx` with shared validator/payload helper at `lib/feedback/payload.ts` and RTL smoke tests; (c) `POST /api/admin/feedback/[id]/to-github` route + per-row "Convert to GitHub issue" button — pure helpers in `lib/feedback/github.ts`, idempotent if `github_issue_url` already set. Requires `GITHUB_TOKEN` env var on Vercel (set 2026-05-13) and `projects.github_repo` set per project (currently editable only via Firebase console — adding it to PATCH allowlist + setup UI is a follow-up). Remaining: (d) Resend inbound webhook → `feedback/{id}/replies` subcollection (`FeedbackReply` type already defined); (e) embed on Bakery Louise repo (`nicolovejoy/bakerylouise-v1`) — plain React, only depends on Tailwind tokens.
