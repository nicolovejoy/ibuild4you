...
1. **#120 — builder Conversations reader-first fix (V1). NEXT.** Transcript becomes the page: chronological, auto-scrolled, read-only (**no composer** — Nico confirmed it's never used), status strip w/ full maker roster, one state-aware dispatch action, agent setup off the screen, tokens/costs → admin-only detail. Chosen over larger redesigns in the 2026-07-10 design round (3 mockups in session).
2. **#121 — decision provenance (after #120).** Stamp `decided_in_session`/`decided_at` on `brief.decisions[]` at regen time (additive schema). Byside seat confirmed immediate value ("when did we lock X" currently answered by grepping transcripts; hand-copied decisions demonstrably drift). Unlocks #122's round headers.
3. **#122 — `inherits_from` + round-timeline view. Gated, not scheduled.** Inherit-not-merge decided 2026-07-10 (locked decisions + constraints travel with provenance; old brief stays archived/linkable). Gates: inheritance at the next new-brief-with-predecessor moment (live test pair exists in the byside family); timeline only if briefs live past ~5 rounds.
...
5. **#107 preview test-login — code done; one owner step remains.** Shared login helper (`scripts/lib/preview-login.mjs`) + docs shipped. Remaining: wire Google OAuth on preview (owner-only console work — see #107 for the exact Firebase/GCP steps). Passcode login already works; Google is only for a human eyeballing preview.
6. **#84 authoring pivot (north star).** Manual Agent-setup forms read as superfluous; real workflow is chat-with-Claude → paste JSON blob. De-emphasize hand-edited forms toward conversational/JSON authoring. Complex; direction-of-record, not next.
7. **🔥 OPERATIONAL RUNBOOK — brief-regen cost runaway (fixed PR #78, keep this).** If Anthropic costs spike: query the **`api_usage`** Firestore collection grouped by route+project, look for a project stuck at the cron interval (was `route='brief.generate'` retrying every 5-min tick, ~$8.4/day). Gate lives at `lib/api/brief-regen-gate.ts` (breaker holds after 3 fails); emergency stop: `scripts/stop-regen-loop.mjs <projectId> --apply`.
8. **⚠️ Preview gotchas.** (a) Vercel SSO gate is ON — agent uses `.ibuild4you-bypass` + `.test-admin-passcode` (preview) / `.test-admin-passcode-prod` (prod) for headless Playwright. (b) Google sign-in on preview 400s (`redirect_uri_mismatch`) — never wired; passcode login works (fix in #107). (c) **Email/Password is enabled on both Firebase projects** (`ibuild4you-preview` + `ibuild4you-a0c4d`) as of 2026-07-01 — password login works on preview + prod.
