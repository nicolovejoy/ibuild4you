...
19. **🎨 Brief/Setup UX scrub — Phases 1/2/4 SHIPPED; Phase 3 next.** Full plan + Phase-3 design pass: `docs/ux-scrub-brief-setup.md`. **P1 (PR #92, `d5f9eca`):** killed the Setup config duplication — `EditableSetup`→collapsible `AgentConfigCard` (single config home), identity + github_repo into its Advanced fold, dispatch card pure dispatch. **P2 (PR #93, `fc32a9c`, 2026-06-22):** nav → **Brief · Conversations · People** (`TabId='brief'|'conversations'|'people'`); `ConversationsTab` pins the "Next round" card atop past conversations; `PeopleTab` extracts the roster; Files fold into Brief as an Attachments strip; legacy `?tab=` remaps. **P4 bug (PR #94, `d1a6ddb`, 2026-06-22):** established-maker share modal shows an access-only "Maker access" view (link+passcode+reset), not first-time invite copy — gated on a fresh share this session. **Phase 3 = brief-as-document editor** (structured view + raw-JSON toggle over `BriefContent`; raw view = the copy-paste ferry target/source, [[project_cost_routing]]; optional "Update from conversation (uses API)", never default) — **design pass DONE (in the doc), awaiting Nico's 2 calls:** overwrite-with-confirm vs field-merge; locked-decision editing in structured view vs JSON-only. github_repo final "Brief settings" home still TBD (parked in AgentConfigCard Advanced). Filed **#83** (artifacts model), **#84** (deferred MCP/Claude Code authoring bridge).
