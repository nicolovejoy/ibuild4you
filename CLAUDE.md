...
**Shipped 2026-06-05 (file-attachment fix — read more types, stop silent drops):** PR #48 (`922169a`) to prod. Root cause of Manine's "agent says nothing came through": upload accepted any type but the agent's loader silently dropped what it couldn't read (Word/text/etc.) — the file showed in her Files tab while the agent got text-only. New `lib/files/supported-types.ts` (single source of truth, classifies by content-type AND extension). `/api/files/init` now 415-rejects unsupported types before storing (no ghost files), mirrored client-side in `MakerProjectView`. `loadAttachmentBlocks` returns `{blocks, dropped}`; chat route injects an `<attachment_note>` on the latest drop-bearing turn so the agent surfaces it. Agent now reads text/code/CSV/JSON (inlined) + Word `.docx` (mammoth) on top of PDF+images; `.pptx`/Pages/HEIC/legacy `.doc` degrade to "export as PDF". 572 tests green (TDD). Verified BOTH paths live on preview: `.docx` extraction (read planted "rutabaga") and screenshot/vision (read a planted brief — 80 loaves, Fri 3–7pm, "artisanal" banned). **Follow-ups:** reply to Manine that it's fixed; bigger drag-and-drop zone (drop target is only the composer row — see Backlog); likely subsumes #38 PDF-validation + "validate Matt's PDF flow".
