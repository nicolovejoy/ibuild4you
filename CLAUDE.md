...
**⚠️ `reliability-hardening` branch is DONE and UNMERGED** (2026-08-22, worktree `/Users/nico/src/ibuild4you-reliability`, 8 commits off `e169d7c`). 1404 tests, build compiles, final review says ready to merge; nothing pushed. Contents: (1) `lib/api/cron-auth.ts` — all four cron routes now fail CLOSED on a missing `CRON_SECRET`; the old `if (secret && ...)` shape made them **publicly callable** whenever that env var went missing, a remotely-triggerable path into the brief-regen billing runaway. (2) `garmProbe()` + a `garm_reachable` check in `/api/health` — Garm gates sign-in fail-closed, so a Garm outage means nobody can sign in, and health used to stay green straight through it. (3) `/api/cron/garm-reconcile`, hourly, gated on `GARM_DUAL_WRITE` — heals missing Garm grants from Firestore membership, additive-only, hard cap of 10. Plan + every ruling: `docs/reliability-hardening-plan.md`. **Merge it before starting anything else here**, or the worktree rots.

**🔴 OPEN — a maker is still locked out, and it is an IDENTITY MISMATCH, not a dropped grant.** A maker could not sign in for 3+ days (denials 08-19/21/22). First diagnosis blamed the dual-write; **that was wrong**, corrected 2026-08-22 once read-only prod Firestore access was restored. A full reconcile diff says **33 expected grants, 34 active, zero missing** — the dual-write has no observed failures. What actually happened: the brief and Garm grant are under the maker's ISP address (grant created correctly by `ibuild4you-dual-write`), and they are signing in with a *Google* address that has no membership and no `approved_emails` row. The system has never seen the address they present.

This is the case aliasing (#169) exists for — one person, two addresses, one identity — same shape as the earlier alias already in production.

**⚠️ A stray grant is currently blocking the fix.** During the wrong diagnosis a grant was hand-created for the Google address (actor `nico`). It lets them authenticate but they hold no membership under it, so they would sign in and see **no briefs** — worse than the original symptom, because the brief looks deleted. It also blocks the real fix: Garm refuses with 409 to alias an address holding an active grant. **Sequence: revoke that grant first, then request the alias Google-address → ISP-address from Garm.** Do NOT "fix" this by adding a second `project_members` row for the second address — that splits one human into two participants in the chat roster and divides message attribution.
