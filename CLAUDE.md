...
16. **Matt/BySide maker feedback — #69/#74/#75/#70 all SHIPPED to prod (2026-06-15).** **#70 SHIPPED (PR #76, `672303f`):** welcome-replay fixed — `app/api/sessions/route.ts` inserts the canned `welcome_message` only on the project's first session; return sessions start empty and the kickoff recaps (`lib/agent/kickoff.ts` + kickoff route now judge prior maker history at the **project** level). Playwright-verified on preview. (#69/#74/#75 shipped earlier same day.) **NEXT = #71 brief↔build reconciliation** (MVP: durable locked-decisions + forced reconciliation pass; durability via a code-side merge in `regenerateBriefForProject` — investigation done this session) and **#72 agent-perceives-prototype via Loop context-capture** (slice A = recent `feedback` rows into the system prompt; NOT headless browsing).
...
19. **🎨 Brief/Setup UX scrub — DIRECTION AGREED, not yet built.** Full plan: `docs/ux-scrub-brief-setup.md`. Decisions (2026-06-18): nav → **Brief · Conversations · People** (drop Setup/Files/Sessions names); "Next round" card (the dispatch card) pinned atop **Conversations** = the only agent-config home; kill the duplicate "Agent setup" (EditableSetup) card + nudge-note field + 3-of-4 share entry points; brief edited as **one structured-JSON document** (structured view + raw-JSON toggle over `BriefContent`); Files fold into Brief. **Cost model (load-bearing): the copy-paste JSON ferry STAYS** — it routes builder reasoning onto Nico's Max sub, not metered API ([[project_cost_routing]]). In-app "Update from conversation (uses API)" is optional, never default. **Phase 1 = pure deletion (kill duplication), lowest risk, ready to start.** Filed **#83** (artifacts model), **#84** (deferred MCP/Claude Code authoring bridge).
20. **🎨 Mode system + session cost — DESIGN AGREED (mode) / PR OPEN (cost), 2026-06-20.** Spec: `docs/mode-system.md`. **Mode system** (not built): signal a viewer's *per-brief* role visually — roles are per-brief, not per-user, so the dashboard is a shared landing where you're Originator on one brief and Reviewer on another (today every card mis-reads "Reviewer" because `defaultBriefRole('owner')→null` falls through). Two channels, **never merged**: brief identity (color+code+glyph = which brief, viewer-independent, on OG) vs **mode** (your relationship). Two granularities, both from role: a **one-per-role glyph** (studio family 🎤 Originator / 🎸 Contributor / 🎛️ Reviewer, + future 🎧 observer / 🎬 approver) and a coarse **participant/operator chrome** split (conversation vs **Console**) via `resolveMode`. NOT card-bg/hatch (a11y + conflicts with identity). Phasing: P1 glyph map + `resolveMode` + dashboard glyph (low risk); P2 dramatic chrome divergence (+ `UserMenu` dark variant — the one cross-boundary coupling); P3 OG/SVG + future roles. **Session cost (PR #88, OPEN):** per-session `token_cost_usd` shown as `· ~$X` next to the token count; accumulated from FULL usage (incl. cache) via tested `lib/observability/session-cost.ts` so it's accurate even though stored `token_usage_input` is the uncached remainder. Validate: `token_cost_usd` == sum(`api_usage.cost_usd` per session) + vs Console. (PR #88 also carries the mode-system doc.)
