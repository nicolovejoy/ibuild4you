...
1. **RAAC Phase 1+2 — validate on preview, push to main.** Local `main` is 2 commits ahead of `origin/main` (`65e443d` RAAC Phase 1+2, `27129a9` About brief/voice sections + nav). Both pushed to `origin/preview` (force). Eyeball on `preview.ibuild4you.com`: About page (`/about`), maker chat header (Roan avatar + "Chatting with Roan"), new session opener (Roan's identity), dashboard nav (About link in header + UserMenu dropdown). When good: `git push origin main`. Live makers will see the agent voice flip to Roan on next visit — that's intentional but worth a final gut-check.
2. **RAAC Phase 3a — `brief_role` schema field + backfill.** Add `brief_role: 'originator' | 'contributor' | 'reviewer'` to `project_members` alongside the legacy `role` access tier. Backfill script: maker→originator, builder→reviewer, apprentice→contributor. API routes (`share`, `claim`, `role`, POST/PATCH projects) accept the new field. No UI change yet — that's 3b. Full plan at `~/.claude/plans/quiet-shimmering-roan.md`. Unblocks #44 (dashboard restructure under RAAC).
3. **Voice attribution implementation (issue #43).** Per the research-agent findings appended on the issue: function-based prose disclosure on About page (one footer line) + section-level provenance strips on brief sections (`authored_by`, `drafted_by`, `last_edited_by`, `last_edited_at`). About-page piece is small; brief-section piece is a small schema migration + UI strip. Avoid per-sentence/per-paragraph badges per over-marking research.
4. **🔒 Preview Firestore split — committed.** Provision a separate Firebase project (`ibuild4you-preview` or similar) so preview writes land in their own sandboxed DB instead of overwriting prod. Tasks: create the project; copy Firestore rules + composite indexes from prod; set up Google OAuth (callback URL on the new project) + passcode auth; swap `FIREBASE_*` env vars on Vercel's "Preview" environment only; add `.env.preview.local.tpl` with 1Password refs for local preview-environment dev; optionally seed canonical test projects. The hard part is mechanical not conceptual. Decision drivers: May 23 incident (real maker's `requester_email` overwritten from preview); unblocks agent-driven Playwright on preview (backlog item silently assumes preview is safe to write). ~half day.
...
**Shipped 2026-05-27 (RAAC Phase 1+2 → preview):** Two commits on local `main`, both also force-pushed to `origin/preview` for validation. **65e443d** (Phase 1+2): About page rewritten around RAAC vocabulary (Brief / Roan / Originator / Contributor / Reviewer) with hero illustration; agent identity flipped to Roan in `lib/agent/constants.ts` DEFAULT_IDENTITY + system prompt "## Current brief" heading; "Maker direction wins" guardrail → "Their direction wins"; chat header in `MakerProjectView` shows the Roan avatar next to "Chatting with Roan"; message author labels read `copy.chat.agentLabel` ("Roan"); two user-visible "project" leaks ("Project brief", "Project link") fixed. Legacy glossary keys (maker/builder/agent/conversation/nextConversation) deliberately kept in `lib/copy.ts` alongside new RAAC keys — in-UI tooltips still resolve until Phase 3 sweep. **27129a9** (About refinement + nav): new "The brief" section between Roan and Roles, new "A note on voice" section after Vision (flagging human-vs-AI authorship as an open product question), tone harmonized with Nico's edits, About link added to dashboard header + UserMenu dropdown. Issues filed: #43 (voice attribution research — agent-generated findings appended recommending function-based prose disclosure + section-level provenance strips), #44 (dashboard restructure under RAAC, blocked on Phase 3a). Phase 3 plan: `~/.claude/plans/quiet-shimmering-roan.md`. 506/506 tests + type-check + lint clean. **Local `main` 2 commits ahead of `origin/main`** pending preview validation.
