...
// The address a liveness probe checks against. Not a real account — chosen so
// the request is well-formed and exercises the full round trip (URL, key,
// project scope, Garm's DB) without depending on any particular grant.
const PROBE_EMAIL = 'health-probe@example.com'
 ...
 *
 * Deliberately does NOT call garmCheck or touch its module-level cache: a probe
 * that runs on a health-check cadence must never seed or evict a real sign-in
 * decision. It issues its own fetch instead.
 */
export async function garmProbe(): Promise<GarmProbeResult> {
  const start = Date.now()

  const url = process.env.GARM_URL
  if (!url) return { ok: false, error: 'GARM_URL not set', ms: Date.now() - start }
  const key = process.env.GARM_KEY
  if (!key) return { ok: false, error: 'GARM_KEY not set', ms: Date.now() - start }
  ...
  try {
    const res = await fetch(`${url}/gnipahellir`, {
      ...
      body: JSON.stringify({ email: PROBE_EMAIL, project: GARM_PROJECT, min_role: 'viewer' }),
      cache: 'no-store',
      signal: AbortSignal.timeout(TIMEOUT_MS),
    })
    if (!res.ok) {
      return {
        ok: false,
        status: res.status,
        error: `garm returned ${res.status}`,
        ms: Date.now() - start,
      }
    }
    ...
    if (typeof data.allowed !== 'boolean') {
      return {
        ok: false,
        status: res.status,
        error: 'garm response missing boolean allowed field',
        ms: Date.now() - start,
      }
    }
