...
// #72 slice B1 — the widget-side page capture. A pure DOM walk that produces a
// structural outline (headings, landmarks, control labels, counts) and NEVER
// user-typed values. Privacy is the contract here: most of these tests assert
...
beforeEach(() => {
  ...
  document.title = 'Checkout — Byside'
})
...
describe('buildPageCapture — basics', () => {
  it('captures version, route (path only) and title', () => {
    ...
    expect(cap.title).toBe('Checkout — Byside')
  })
...
describe('buildPageCapture — privacy (what must NOT leak)', () => {
  ...
  it('excludes hidden and aria-hidden elements', () => {
    setBody('<h2 hidden>Secret A</h2><div aria-hidden="true"><h2>Secret B</h2></div><h2>Visible</h2>')
    const { outline } = buildPageCapture(document, loc)
...
describe('buildPageCapture — caps', () => {
  ...
  it('caps the whole outline', () => {
    const headings = Array.from({ length: 400 }, (_, i) => `<h2>Section ${i} lorem ipsum dolor</h2>`).join('')
    setBody(headings)
    const { outline } = buildPageCapture(document, loc)
