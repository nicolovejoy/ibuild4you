// #72 slice B1 — widget-side page capture for Loop.
//
// A pure DOM walk that turns the page the maker is looking at into a small
...
// non-heading body text, and query strings NEVER appear in the output. Host
// apps can additionally exclude any subtree with a `data-loop-redact` attribute.
//
// This file travels with FeedbackWidget.tsx + payload.ts when Loop is copied
// into a host app (see docs/loop.md) — keep it dependency-free, plain DOM APIs.
...
// Server-side caps mirror these (app/api/feedback/route.ts).
...
export function buildPageCapture(doc: Document, loc: { pathname: string }): PageCapture {
  ...
  // Button labels — what can the maker do on this page?
